...
{
    public class UpdateEnglishPageCommandHandler
        : IRequestHandler<UpdateEnglishPageCommand, CommandResponse>
    {
        private readonly ApplicationDbContext _context;
        private readonly ILogger<UpdateEnglishPageCommandHandler> _logger;
        private readonly IUserAccessor _userAccessor;

        public UpdateEnglishPageCommandHandler(
            ApplicationDbContext context,
            ILogger<UpdateEnglishPageCommandHandler> logger,
        ...
        public async Task<CommandResponse> Handle(
            UpdateEnglishPageCommand request,
            CancellationToken cancellationToken
        )
        {
            var englishPage = await _context.EnglishPage.FirstAsync(cancellationToken);
            ...
            if (await _context.SaveChangesAsync(cancellationToken) > 0)
            {
                _logger.LogInformation(
                    "EnglishPage updated by {UserRealName} in {DoneTime}",
