...
namespace Persistence.CQRS.Captcha
{
    public class GenerateCaptchaCommandHandler : IRequestHandler<GenerateCaptchaCommandRequest, byte[]>
    {
        ...
        public GenerateCaptchaCommandHandler(IWebHostEnvironment environment, ISessionService sessionService)
        {
            _environment = environment;
            _sessionService = sessionService;
        }

        public async Task<byte[]> Handle(GenerateCaptchaCommandRequest request, CancellationToken cancellationToken)
        {
        ...
        private async Task<byte[]> GenerateCaptchaImage(string code)
        {
            ...
            string fontPath = Path.Combine(_environment.WebRootPath, "fonts", "B-NAZANIN.TTF"); ; // مسیر فایل فونت در پروژه
