...
namespace Persistence.CQRS.Resources.Publications
{
    public class CreatePublicationCommandHandler : IRequestHandler<CreatePublicationCommand, CommandResponse>
    {
        ...
        public CreatePublicationCommandHandler(ApplicationDbContext context, ILogger<CreatePublicationCommandHandler> logger, IUserAccessor userAccessor)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<CommandResponse> Handle(CreatePublicationCommand request, CancellationToken cancellationToken)
        {
            ...
            if (await _context.SaveChangesAsync(cancellationToken) > 0)
            {
                ...
                _logger.LogInformation($"Publication with id {entity.Id} created by {_userAccessor.GetUserName()} in {DateTime.Now}");
