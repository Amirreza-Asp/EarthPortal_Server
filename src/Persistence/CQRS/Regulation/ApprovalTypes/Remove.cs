...
{
    public class RemoveApprovalTypeCommandHandler
        : IRequestHandler<RemoveApprovalTypeCommand, CommandResponse>
    {
        private readonly ApplicationDbContext _context;
        private readonly ILogger<RemoveApprovalTypeCommandHandler> _logger;
        private readonly IUserAccessor _userAccessor;

        public RemoveApprovalTypeCommandHandler(
            ApplicationDbContext context,
            ILogger<RemoveApprovalTypeCommandHandler> logger,
        ...
        public async Task<CommandResponse> Handle(
            RemoveApprovalTypeCommand request,
        ...
        )
        {
            ...
            if (await _context.SaveChangesAsync(cancellationToken) > 0)
            {
                _logger.LogInformation(
                    "ApprovalType with id {Id} removed by {UserRealName} in {DoneTime}",
                    entity.Id,
