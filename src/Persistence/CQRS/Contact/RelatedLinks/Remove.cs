...
{
    public class RemoveRelatedLinkCommandHandler
        : IRequestHandler<RemoveRelatedLinkCommand, CommandResponse>
    {
        private readonly ApplicationDbContext _context;
        private readonly ILogger<RemoveRelatedLinkCommandHandler> _logger;
        private readonly IUserAccessor _userAccessor;

        public RemoveRelatedLinkCommandHandler(
            ApplicationDbContext context,
            ILogger<RemoveRelatedLinkCommandHandler> logger,
        ...
        public async Task<CommandResponse> Handle(
            RemoveRelatedLinkCommand request,
        ...
        )
        {
            ...
            if (await _context.SaveChangesAsync(cancellationToken) > 0)
            {
                _logger.LogInformation(
                    "RelatedLink with id {Username} removed by {UserRealName} in {DoneTime}",
                    entity.Id,
